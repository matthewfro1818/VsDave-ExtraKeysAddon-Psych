...
class FunkinLua {
	...
	public function new(script:String) {
		...
		if (CharSelectState.character == 'Default')
			...
			set('boyfriendName', CharSelectState.character);
		...
		set('antimash', !ClientPrefs.noAntimash);
		set('noteSplashesActivated', ClientPrefs.noteSplashes);
		set('hideHud', ClientPrefs.hideHud);
		set('timeBarType', ClientPrefs.timeBarType);
		set('scoreZoom', ClientPrefs.scoreZoom);
		set('cameraZoomOnBeat', ClientPrefs.camZooms);
		set('cameraCharMovement', ClientPrefs.cameraMovement);
		...
		Lua_helper.add_callback(lua, "getColorFromHex", function(color:String) {
			...
			luaTrace('Color is: ' + color);
			return Std.parseInt(color);
		});
		Lua_helper.add_callback(lua, "getColorFromRGB", function(R:Int, G:Int, B:Int) {
			var color:FlxColor = FlxColor.fromRGB(R, G, B);
			var colorString:String = color.toHexString();
			luaTrace('Color is: ' + colorString);
			trace('Color is: ' + colorString);
			return Std.parseInt(colorString);
		});
		Lua_helper.add_callback(lua, "getHealthColor", function(char:String, ?format:String = 'color') {
			var colorArray:Array<Int>;
			var colorReturn:Dynamic;
			switch (char) {
				case 'gf' | 'girlfriend' | '2':
					colorArray = PlayState.instance.gf.healthColorArray;
				case 'dad' | '1':
					colorArray = PlayState.instance.dad.healthColorArray;
				default:
					colorArray = PlayState.instance.boyfriend.healthColorArray;
			}
			if (format.toLowerCase() == 'hex' || format.toLowerCase() == 'color')
			{
				var color:FlxColor = FlxColor.fromRGB(colorArray[0], colorArray[1], colorArray[2]);
				var colorString:String = color.toHexString();
				luaTrace('Color is: ' + colorString);
				trace('Color is: ' + colorString);
				if (format.toLowerCase() == 'color')
					colorReturn =  Std.parseInt(colorString);
				else
					colorReturn = colorString;
			}
			else
				colorReturn = colorArray;
			return colorReturn;
		});
		...
		Lua_helper.add_callback(lua, "getNoteKey", function(variable:Int, ?isSecondKey:Bool = false, ?fromMania:Int) {
			var secondKey:Int;
			if (isSecondKey)
				secondKey = 1;
			else
			...
			if (fromMania != null)
				return PlayState.instance.getNoteKey(variable, secondKey, fromMania);
			else
				return PlayState.instance.getNoteKey(variable, secondKey);
		});
		Lua_helper.add_callback(lua, "setNoteKey", function(variable:Int, key:String, ?key2:String) {
			PlayState.instance.setNewKey(variable, key, key2);
		});
		Lua_helper.add_callback(lua, "changeIcon", function(type:String, name:String) {
			PlayState.instance.changeIconEvent(type, name);
		});
		Lua_helper.add_callback(lua, "changeHealthColor", function(type:String, color:String) {
			PlayState.instance.changeHealthColorEvent(type, color);
		});
		...
		Lua_helper.add_callback(lua, "cameraSetTarget", function(target:String) {
			...
			if(target == '2' || target == 'girlfriend') {
				target = 'gf';
			}
			if(target == '0' || target == 'boyfriend') {
				target = 'bf';
			}
			PlayState.instance.charToFolow = target;
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			#if MODS_ALLOWED
			path = Paths.modsJson(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile + '-' + ClientPrefs.language);
			if(!FileSystem.exists(path))
				path = Paths.modsJson(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile);
			if(!FileSystem.exists(path))
			#end
			path = Paths.json(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile + '-' + ClientPrefs.language);
			if(!FileSystem.exists(path))
		...
		Lua_helper.add_callback(lua, "playLangSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				tag = tag.replace('.', '');
				if(PlayState.instance.modchartSounds.exists(tag)) {
					PlayState.instance.modchartSounds.get(tag).stop();
				}
				PlayState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.langSound(sound), volume, false, function() {
					PlayState.instance.modchartSounds.remove(tag);
					PlayState.instance.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
			FlxG.sound.play(Paths.langSound(sound), volume);
		});
